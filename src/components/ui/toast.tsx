...
const getIcon = (variant: ToastVariant) => {
  switch (variant) {
    ...
    case "error":
      return <Icon name="close" size={20} className="text-destructive" />;
    case "warning":
      return <Icon name="alert" size={20} className="text-attention" />;
    case "info":
...
const createToastContent = (variant: ToastVariant, title?: string, description?: string) => {
  return (
...
export const toast = {
  success: (title: string, options?: ToastOptions) => {
    return sonnerToast.custom(() => createToastContent("success", title, options?.description), {
  ...
  error: (title: string, options?: ToastOptions) => {
    return sonnerToast.custom(() => createToastContent("error", title, options?.description), {
  ...
  warning: (title: string, options?: ToastOptions) => {
    return sonnerToast.custom(() => createToastContent("warning", title, options?.description), {
      duration: options?.duration || 4000,
      action: options?.action,
    });
  },
  ...
  info: (title: string, options?: ToastOptions) => {
    return sonnerToast.custom(() => createToastContent("info", title, options?.description), {
      duration: options?.duration || 4000,
      action: options?.action,
    });
  },
  ...
  default: (title: string, options?: ToastOptions) => {
    return sonnerToast.custom(() => createToastContent("default", title, options?.description), {
      duration: options?.duration || 4000,
      action: options?.action,
    });
  },
