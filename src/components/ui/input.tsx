...
export const Input = React.forwardRef<HTMLInputElement, InputProps>(({ className, type, label, leftIcon, rightIcon, onRightIconClick, showPasswordToggle = false, mask, onMaskedChange, onChange, noWrapper = false, ...props }, ref) => {
  ...
  const renderIcon = (iconName: IconName | "instagram" | "facebook", size: number = 18) => {
    if (iconName === "instagram") {
      return <img src={instagramIcon} alt="Instagram" width={size} height={size} className="object-contain" />;
    }
    if (iconName === "facebook") {
      return <img src={facebookIcon} alt="Facebook" width={size} height={size} className="object-contain" />;
    }
    return <Icon name={iconName as IconName} size={size} />;
  };

  const hasRightElement = Boolean(rightIcon) || (showPasswordToggle && type === "password");

  const inputElement = (
    <div className="relative">
      {leftIcon && <div className="absolute left-3 top-1/2 -translate-y-1/2 text-foreground z-10 pointer-events-none">{renderIcon(leftIcon, 18)}</div>}
      ...
      <input
        ...
        className={cn(
          "flex h-12 w-full rounded-lg border-2 border-slate-200 bg-white px-3 py-2.5 text-base text-slate-700 placeholder:text-slate-400 transition-all duration-200 outline-none shadow-sm",
      ...
      {hasRightElement && (
        <button type="button" onClick={handleRightIconClick} className="absolute cursor-pointer right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors focus:outline-none focus:text-foreground z-10" tabIndex={-1}>
          {rightIcon && rightIcon !== "instagram" && rightIcon !== "facebook" ? renderIcon(rightIcon, 18) : <Icon name={getRightIcon()} size={18} />}
        </button>
      )}
    </div>
  );
