...
import { getProspectors, createProspector, updateProspector, toggleProspectorStatus, deleteProspector, type User, type CreateUserData, type UpdateUserData } from "@/services/users";
...
const Prospectors = () => {
  ...
  const [prospectorToDelete, setProspectorToDelete] = useState<User | null>(null);
  const [prospectorToToggleStatus, setProspectorToToggleStatus] = useState<User | null>(null);
  ...
  const updateProspectorMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: UpdateUserData }) => updateProspector(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["prospectors"] });
      setIsSheetOpen(false);
    ...
    onError: (error: Error & { response?: { data?: { message?: string } } }) => {
      toast.error("Erro", {
        description: error.response?.data?.message || "Erro ao atualizar parceiro",
        duration: 3000,
      });
    },
  });
  ...
  const toggleStatusMutation = useMutation({
    ...
    onError: (error: Error & { response?: { data?: { message?: string } } }) => {
      toast.error("Erro", {
        description: error.response?.data?.message || "Erro ao alterar status do parceiro",
        duration: 3000,
      });
    },
  });
  ...
  const deleteProspectorMutation = useMutation({
    ...
    onError: (error: Error & { response?: { data?: { message?: string } } }) => {
      if (error?.response?.data?.message && error?.response?.data?.message !== "404") {
        toast.error("Erro", {
  ...
  const ProspectorCard = ({ prospector, onEdit, onDelete, onToggleStatus }: ProspectorCardProps) => {
    return (
      <Card className="p-4">
        <div className="flex items-start justify-between">
          <div className="space-y-2 flex-1">
            <div className="flex items-center gap-2">
              ...
              <button type="button" className="hover:opacity-70 transition-opacity" onClick={() => onToggleStatus(prospector)}>
                <Badge variant={prospector.active ? "default" : "secondary"}>{prospector.active ? "Ativo" : "Inativo"}</Badge>
              </button>
            </div>
            <p className="text-sm text-muted-foreground">{prospector.email}</p>
            {prospector.phone && <p className="text-sm text-muted-foreground">{prospector.phone}</p>}
            {prospector.birthdate && <p className="text-sm text-muted-foreground">Nascimento: {new Date(prospector.birthdate).toLocaleDateString("pt-BR")}</p>}
            <p className="text-xs text-muted-foreground">Criado em: {new Date(prospector.createdAt).toLocaleDateString("pt-BR")}</p>
          </div>
          <div className="flex gap-2">
            <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={() => onEdit(prospector)}>
              <Icon name="settings" size={16} />
            </Button>
            <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-destructive hover:text-destructive" onClick={() => onDelete(prospector)}>
  ...
  return (
    <>
      <AppHeader title="Gerenciamento de Parceiros" showActionButton={false} />

      <div className="flex h-16 shrink-0 items-center gap-2 border-b px-4">
        <div className="ml-auto flex items-center gap-3">
          <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
            <Button size="sm" className="bg-primary hover:bg-primary/90" onClick={handleNewProspector}>
              <Icon name="plus" size={20} className="mr-2 text-foreground" />
              <span className="text-foreground font-bold text-md">Novo Parceiro</span>
            </Button>
            <SheetContent className="min-w-[400px] sm:min-w-[540px] p-4">
              <SheetHeader>
                <SheetTitle className="text-xl">{editingProspector ? "Editar Parceiro" : "Cadastrar Novo Parceiro"}</SheetTitle>
                <SheetDescription>{editingProspector ? "Atualize os dados do parceiro." : "Preencha os dados para cadastrar um novo parceiro."}</SheetDescription>
              </SheetHeader>
              <form onSubmit={handleSubmit} className="space-y-4 mt-6">
                ...
                <div className="flex gap-3 pt-4">
                  <Button type="submit" className="flex-1" disabled={createProspectorMutation.isPending || updateProspectorMutation.isPending}>
                    <Icon name="check" size={16} className="mr-2 text-white" />
                    <span className="text-white font-bold text-lg">{editingProspector ? "Atualizar" : "Cadastrar"}</span>
                  </Button>
      ...
      <div className="flex flex-1 flex-col gap-6 p-6">
        <div className="space-y-2">
          <h2 className="text-2xl font-bold tracking-tight text-slate-800">Lista de Parceiros</h2>
          <p className="text-slate-600">Gerencie todos os parceiros prospectores do sistema</p>
        </div>
          ...
          {isMobile ? (
            <div className="space-y-4">
              {isLoading ? (
                ...
                prospectors.map((prospector) => <ProspectorCard key={prospector.id} prospector={prospector} onEdit={handleEditProspector} onDelete={handleDeleteProspector} onToggleStatus={handleToggleUpdateProspectorStatus} />)
              )}
            </div>
          ) : (
            <DataTable columns={columns} data={prospectors} isLoading={isLoading} />
          )}
        </div>
      </div>
      <Dialog
      ...
      <Dialog
        title={prospectorToToggleStatus?.active ? "Desativar Parceiro" : "Ativar Parceiro"}
